...
/*----------  Communication  ----------*/
void serial_send() {
	int TX_free_bytes = Serial.AvailableToWrite();
	if(TX_free_bytes > 0) {
		digitalWrite(PIN_LED, LOW);
		
		Serial.write(); // (TODO) Add output bytes to TX queue
...
void loop() {
	...
	while(len(measuredists_shortbuffer) < BATCH_SIZE) {
		// Get a point and it's data (distance, time and last tick index to get a reference for the angle.)
		measuredists_shortbuffer      .append(LidarLite.distance(len(measuredists_shortbuffer) == 0 ? true : false));
		measuretimes_shortbuffer      .append(millis() - lap_start_time);
		measures_last_tick_shortbuffer.append(lap_ticks_count);
